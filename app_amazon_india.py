...
# ===================== TÍTOL I ESTADÍSTIQUES =====================

st.title("📊 Anàlisi Visual de Productes d'Amazon Índia")
st.markdown("Aquest dashboard mostra la relació entre preus, descomptes, valoracions i categories de productes.")
...
with st.expander("1. Preu vs Valoració per Categoria"):
    ...
    fig1 = px.scatter(df_sample, x='discounted_price', y='rating',
                      ...
                      title="Relació entre Preu i Valoració")
    st.plotly_chart(fig1, use_container_width=True)

with st.expander("2. Anàlisi Comparativa per Categories"):
    ...
    fig2 = px.bar(cat_stats, x='main_category', y='rating', title="Valoració Mitjana per Categoria")
    st.plotly_chart(fig2, use_container_width=True)

with st.expander("3. Efectivitat dels Descomptes"):
    fig3 = px.box(df_clean, x='discount_range', y='rating', title="Descomptes vs Valoració")
    st.plotly_chart(fig3, use_container_width=True)

with st.expander("4. Distribució de Preus"):
    fig4 = px.histogram(df_clean, x='discounted_price', nbins=50, title="Distribució de Preus")
...
with st.expander("5. Tendències de Valoració per Preu"):
    fig5 = px.violin(df_clean, x='price_range', y='rating', title="Valoracions per Rang de Preu")
